...
count2<-1
count3<-1
# finally, we need variables for the implementation of the choice. We initialise them at 0.33
choice1<-0.33
choice2<-0.33
choice3<-0.33
# initialise PE
PE<-NULL
# initialise variable that stores choice, as sampling from the three probabilities
actualChoice<-NULL
# initialise the performacne, as the amount of time participants choose the most probable category
performance<-NULL
# loop through the trials
for (t in 1 :20){
  # choice by participant, as sampling from a distribution 
  actualChoice[t]<-sample(x=c(1,2,3), size= 1,prob=c(choice1[t],choice2[t],choice3[t]))
  # now we want to compute the feedback according to the choice. Is the choice equal to the one that will be shown and thus correct feedback?
  if (actualChoice[t]==objCat[t]){ 
    r[t]<-1}else{r[t]<-0}
  
  # now, depending on the object category that is shown, the valu, strength of the association of that category with the scene will change
  if (objCat[t]==1){
  ...
  choice1[t+1]<-exp(beta *tail(Q1,1))/(exp(beta *tail(Q1,1))+exp(tail(Q2,1))+exp(beta *tail(Q3,1))) #the tail functions determines the last value of the array
  choice2[t+1]<-exp(beta *tail(Q2,1))/(exp(beta *tail(Q1,1))+exp(tail(Q2,1))+exp(beta *tail(Q3,1)))
  choice3[t+1]<-exp(beta *tail(Q3,1))/(exp(beta *tail(Q1,1))+exp(tail(Q2,1))+exp(beta *tail(Q3,1)))
  # preference for the category
  performance[t]<- length(actualChoice[actualChoice==1])/t 
}
