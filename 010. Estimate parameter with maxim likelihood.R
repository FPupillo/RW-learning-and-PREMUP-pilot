...
#parameterEstimationPhase1maximumLikel<-function(){
 ...
 printplot<-readline("Would you like to print the plots? [y/n]")  
  ...
  source("005. Parameter estimation function w Likelihood.R")
  # select only phase 1
  ...
  Parameters<-matrix(NA, nrow = length(phase1Files),ncol = 13) 
  colnames(Parameters)<-c("PartNum", "alpha4","perf4","PE4", "absPE4" ,"alpha5", "perf5","PE5",
                          "absPE5","alpha6",  "perf6", "PE6", "absPE6") #names of the columns
  # set the margins for the graphs

  if (printplot=="y"){
    # Open a pdf file
    pdf("Phase1maxlike.rplot.pdf")
  }
  par(mfrow=c(5,3))
  par(mar = c(4,2,2,1))
  ...
  for (j in 1 :  length(phase1Files)){
    ...
    for (s in 4:6){ # we are o
      ...
      # estimate alpha parameter with maximum likelihood
      Par<-maxlikel(c)[[1]]
      # estimate PE. Include alpha parameter, and 10 as beta
      PE<-findlikel(Par,10)[[10]]
      absPE<-findlikel(Par,10)[[11]]
      
      Parameters[j,c(1, coln, coln+1, coln+2, coln+3)]<-c(round(as.numeric(substr(phase1Files[j],5,6)),1), Par, tail(preference,1),mean(PE), mean(absPE))
      ...
      abline(h=c(-0.2,-0.4,-0.6,-0.80,0,0.2,0.4,0.6,0.8), lty=1, col="grey")#for unsined PE
      ...
      title(main = paste("alpha = ", round(Par[[1]],2)),line=0.1)
  ...
  if (printfile=="y"){
    write.csv(dataframe, "/home/francesco/Desktop/PowerFolders/Frankfurt_University/Computational models/RW, Bayes, and PREMUP pilot/participants' parameters estimate max likel.csv")
  }
