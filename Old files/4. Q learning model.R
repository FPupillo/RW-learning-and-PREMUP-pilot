##Q learning algorithm for associative learning implementation, fit to individual data. Estimate parameters with prediction error. 
#murray et al, 2007. Substantia nigra/ventral tegmental reward prediction error disruption in psychosis
...
#it can also be 1 or 0 depending on the reward
...
for (j in 1 :  length(phase1Files)){
  ...
  Qfunction<-function(alpha, beta, lambda, strength){
  lambda<-NULL
  ...
  strength<-Qfunction( alpha,beta, lambda,strength)[[1]]
  prediction_error<-Qfunction( alpha,beta, lambda,strength)[[2]]
